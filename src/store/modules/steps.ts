...
export default {
  ...
  getters: {
    sideBenchs: state => ({ side }) => {
      return state.list.filter(step => step[2] === side && step[3] === TYPES.BENCH);
    },
    sideCommons: state => ({ side }) => {
      return state.list.filter(step => step[2] === side && step[3] === TYPES.COMMON);
    },
    sideEndpoints: state => ({ side }) => {
      return state.list.filter(step => step[2] === side && step[3] === TYPES.ENDPOINT);
    },
